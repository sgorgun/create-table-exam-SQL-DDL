...
namespace AutocodeDB.Parsers
{
    ...
    {
        private static readonly Regex TableNameMatcher = new (@"\s*CREATE\sTABLE\s\[?(?<tblName>[A-Za-z_]*)\[?\s*",
            RegexOptions.Compiled|RegexOptions.IgnoreCase); 
        private static readonly Regex ForeignKeyRegExp = new (@"\s+FOREIGN\s+KEY\s*[(]\s*(?<localId>[A-Za-z_]*)\s*[)]\s*REFERENCES\s+(?<refTable>[A-Za-z_]*)\s*[(]\s*(?<refId>[A-Za-z_]*)\s*[)]",
            RegexOptions.Compiled|RegexOptions.IgnoreCase);
        private static readonly Regex ConstraintRegExp = new (@"\s*CONSTRAINT\s",
            RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static readonly Regex ColumnRegExp = new (@"\s*\[?\s*(?<colName>[A-Za-z_]*)\s*\]?\s+(?<colType>[A-Za-z_]*)\s*",
            RegexOptions.Compiled | RegexOptions.IgnoreCase);
